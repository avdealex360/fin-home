...
8. [Вклад и калькулятор](#8-калькулятор-вклада)
9. [Кошелёк USDC (ERC-20)](#9-кошелёк-usdc-erc-20)
...
│   └── wallet.py        # Кошелёк USDC: настройки + баланс
└── services/            # Бизнес-логика (без FastAPI)
    ...
    ├── analytics.py
    ├── deposit_calc.py
    ├── crypto_wallet.py # Баланс USDC через Etherscan + уведомление в Telegram
    └── exchange.py
```
...
│   ├── wallet.ts        # Статус кошелька USDC для переворота на Главной
...
- Тап по большому числу переворачивает его на **баланс кошелька USDC**, если кошелёк настроен (см. §9).
...
## 9. Кошелёк USDC (ERC-20)

Справочная интеграция для зарплаты в USDC: **на бюджет и 50/30/20 не влияет**, операций не создаёт. Отдельный раздел «Ещё» → «Кошелёк USDC» (`#/wallet`), доступный **любому участнику** — адрес и ключ Etherscan живут в пределах workspace, права админа не нужны. Подробная инструкция — [usdc-wallet-setup.md](usdc-wallet-setup.md).

Что делает:

- фоновый воркер (`crypto_wallet.poll_loop`, задача в `lifespan`) раз в 5 минут спрашивает у **Etherscan V2** баланс USDC на заданном адресе Ethereum mainnet и кладёт его в `Setting`;
- на Главной тап по большому числу переворачивает карточку с рублёвого баланса на баланс кошелька (анимация `rotateX`), на обратной стороне — адрес маской, время обновления, порог и кнопка «Обновить»;
- когда баланс `≥` порога — **один раз за календарный месяц** уходит сообщение в Telegram («похоже, зарплата пришла»). Факт отправки хранится в `wallet.alert_month`, поэтому опрос каждые 5 минут не превращается в спам.

Настройки (`Setting`): `wallet_address`, `wallet_threshold`, `wallet_notify_user_id` — в пределах workspace; кэш — `wallet.balance`, `wallet.checked_at`, `wallet.error`, `wallet.alert_month`; ключ Etherscan — install-wide секрет `secret.etherscan_api_key` (не попадает в экспорт, наружу отдаётся только флагом «задан»). Полный адрес API не отдаёт — только маску `0x5564…3148`. Миграций фича не требует.
