...
Справочная интеграция для зарплаты в USDC: **на бюджет и 50/30/20 не влияет**, операций не создаёт. Настраивается в «Ещё» → «Телеграм-бот, AI и кошелёк» (только админ). Подробная инструкция — [usdc-wallet-setup.md](usdc-wallet-setup.md).
