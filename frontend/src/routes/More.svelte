<script lang="ts">
  ...
  const GLOSSARY = [
    ...
    { term: 'Кошелёк USDC', def: 'Если в разделе «Кошелёк USDC» задан адрес и ключ Etherscan — тап по большому числу на Главной переворачивает его на баланс кошелька. Бот пишет раз в месяц, когда баланс превысил порог.', formula: 'баланс USDC ≥ порог → одно сообщение в месяц' },
  ]
</script>
