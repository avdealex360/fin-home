  ...
  <div class="cols">
    <section class="col-wide stack">
      ...
      <div class="card">
        ...
        {#if $showHelp}
          <p class="explain">Деньги на одном счёте, но мысленно разложены по целям — так проще не потратить подушку на отпуск.</p>
        {/if}
        {#if funds.length === 0}
      ...
      <div class="card">
        ...
        {#if $showHelp}
          <p class="explain">Гасить выгоднее тот, у которого выше ставка — он растёт быстрее остальных.</p>
        {/if}
        {#if debts.length === 0}
