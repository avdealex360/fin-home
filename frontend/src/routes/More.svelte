<script lang="ts">
  ...
  let LINKS = $derived([
    ...
    ...($me?.is_admin
      ? [
          { href: '#/integrations', icon: 'ti-robot', name: 'Телеграм-бот, AI и кошелёк', meta: 'Запись трат сообщением, баланс USDC' },
  ...
  const GLOSSARY = [
    ...
    { term: 'Кошелёк USDC', def: 'Если в «Интеграциях» задан адрес и ключ Etherscan — тап по большому числу на Главной переворачивает его на баланс кошелька. Бот пишет раз в месяц, когда баланс превысил порог.', formula: 'баланс USDC ≥ порог → одно сообщение в месяц' },
  ]
</script>
