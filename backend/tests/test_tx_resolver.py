...
def db():
    ...
    s.add_all([
        Category(workspace_id=WS, name="Продукты и быт", group="needs", sort_order=1),
        Category(workspace_id=WS, name="Кофе", group="wants", sort_order=2),
        AppUser(workspace_id=WS, name="Леша", telegram_id="111"),
        AppUser(workspace_id=WS, name="Катя", telegram_id="222"),
        AppUser(workspace_id=WS, name="Общий"),
    ])
    s.commit()
    yield s
    s.close()
...
def test_resolve_category_exact_and_substring(db):
    assert tx_resolver.resolve_category_id(db, WS, "Кофе") is not None
    # substring / case-insensitive
    assert tx_resolver.resolve_category_id(db, WS, "продукты") is not None
    assert tx_resolver.resolve_category_id(db, WS, "Ракета") is None
    assert tx_resolver.resolve_category_id(db, WS, None) is None


def test_resolve_user_defaults_to_sender(db):
    sender = db.query(AppUser).filter_by(name="Леша").first()
    assert tx_resolver.resolve_user_id(db, WS, None, sender) == sender.id
    katya = db.query(AppUser).filter_by(name="Катя").first()
    assert tx_resolver.resolve_user_id(db, WS, "Катя", sender) == katya.id
...
def test_create_transactions_writes_and_marks_unresolved(db):
    ...
    txs = tx_resolver.create_transactions(db, WS, entries, sender)
    assert len(txs) == 2
...
def test_resolve_user_uppercase_keyword_common(db):
    ...
    assert tx_resolver.resolve_user_id(db, WS, "ОБЩЕЕ", sender) == common.id


def test_resolve_category_yandex_group_suffix(db):
    db.add(Category(workspace_id=WS, name="Квартира и жилье", group="needs", sort_order=10))
    db.commit()
    assert tx_resolver.resolve_category_id(db, WS, "Квартира и жилье (needs)") is not None
...
def test_create_transactions_fills_comment_from_source_when_ai_omits(db):
    ...
    txs = tx_resolver.create_transactions(db, WS, entries, sender, source_text=source)
...
def test_resolve_category_group_token_with_source_hint(db):
    ...
    db.add_all([
        Category(workspace_id=WS, name="Квартира и жилье", group="needs", sort_order=10),
        Category(workspace_id=WS, name="Рестораны и доставка", group="wants", sort_order=11),
        Category(workspace_id=WS, name="Связь и интернет", group="needs", sort_order=12),
    ])
    db.commit()
