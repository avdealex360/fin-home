...
def test_digest_caches_for_the_hour(db, monkeypatch):
    ...
    first = daily_digest.get_or_build(db, now=datetime(2026, 7, 2, 10, 15))
    second = daily_digest.get_or_build(db, now=datetime(2026, 7, 2, 10, 45))
    assert first == second
...
def test_digest_stats_always_fresh(db, monkeypatch):
    ...
    first = daily_digest.get_or_build(db, now=datetime(2026, 7, 2, 10, 0))
    ...
    second = daily_digest.get_or_build(db, now=datetime(2026, 7, 2, 10, 30))
...
def test_digest_falls_back_to_static_tip(db, monkeypatch):
    ...
    text = daily_digest.get_or_build(db, now=datetime(2026, 7, 3, 9, 0))
...
def test_digest_rebuilds_on_new_day(db, monkeypatch):
    ...
    a = daily_digest.get_or_build(db, now=datetime(2026, 7, 2, 10, 0))
    b = daily_digest.get_or_build(db, now=datetime(2026, 7, 4, 10, 0))
...
def test_digest_rebuilds_on_new_hour(db, monkeypatch):
    ...
    monkeypatch.setattr(daily_digest, "complete_with_fallback", fake_complete)
    a = daily_digest.get_or_build(db, now=datetime(2026, 7, 2, 10, 59))
    b = daily_digest.get_or_build(db, now=datetime(2026, 7, 2, 11, 0))
