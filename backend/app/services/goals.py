...
# Marker stored in Goal.linked_account_name for goals whose money sits on the
# capitalizing deposit (so their forecast uses compound interest, not linear).
DEPOSIT_ACCOUNT_MARKER = "Вклад"


def _is_deposit_goal(goal: Goal) -> bool:
    return (goal.linked_account_name or "").strip().lower() == DEPOSIT_ACCOUNT_MARKER.lower()
...
def months_to_goal_with_capitalization(
    ...
    if _is_deposit_goal(goal) and goal.deadline:
